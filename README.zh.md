...
![在 github project 里给 Agent 建 issue，review PR](./docs/github_project.png)

一个用 GitHub label 驱动的 **AI coding agent 工作流**（支持 Claude Code / OpenCode / Codex / 你自己的 driver）。以 [Agent Skill](https://agentskills.io/home) 形式安装在你本机——后台每 60 秒轮询一次 GitHub，看到约定的标签触发，就在独立 git worktree 里起一个本机 AI worker，让它做设计 / 写代码 / review 的活，过程和结果通过 GitHub 评论和 PR commit 留痕。

三个前置认知：

- **本机跑，不是云服务**：AI worker 就是你本机的 `claude`（或其他支持的 coding CLI）
- **GitHub 当 UI**：所有触发靠 label 标签（`pending/agent` / `pending/human` / `doing/agent`），没有要学的新 web UI
- **你掌控权**：所有改动都通过 PR review 由你签字才合入；工具从不自动 merge

## 它好在哪

平时跟 AI 写代码是**串行**的：发 prompt → 等回复 → 看 → 反馈 → 等 → 看……一步都不能走开，并且 AI 的聊天历史淹没在茫茫的 session 里难以查找。
...
1. **先看 GitHub issue/PR 评论**——设计提案、讨论过程、review 报告都在这。`gh issue view 42 --comments` 或浏览器，5 秒搞定，**95% 的"这块代码为啥这样"问题都在这一层解决**
...
这是真正长期复利的部分——**决策历史可回溯，人 + AI agent 都能读**。
...
| 长期跑 6 个月（每周几十个 issue） | 几百到几千刀的额外费用　　　 | 原本就在用的 coding agent subscription　　　　　 |

特别适合「24/7 AI 自动 review + 自动 fix」场景——webhook + API 方案 6 个月成本上千刀，本工具只有原本的月费固定成本。
...
- 建项目专属配置
- 登记后台轮询
- 在 GitHub 仓库建好 `pending/agent` / `pending/human` 等标签
- 启动定时任务。
...
- `jq`
- `flock`
- `claude code`（或其他 coding CLI）。

测过 Ubuntu 22.04 / 24.04；macOS 支持，欢迎社区测试。
