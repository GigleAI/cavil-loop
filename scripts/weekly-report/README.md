...
5. **口径切换那一周是部署事实，不能由展示窗口算**。原来在当前 10 周窗口里现找「第一个
   有交叉 review 记账的周」，窗口每周前滚一格，真实切换周滚出去之后，窗口里第一个有记录
   的周就被当成新的切换点——同一份数据、窗口挪一周，报告里的切换日期就从 2/17 变成 2/24，
   图上的红线跟着漂。现在由 `collect.py` 一次性算出 `switch_week` 写进 data.json，报告与
   趋势图**读同一个字段**；配置里的 `WEEKLY_REPORT_SWITCH_WEEK` 优先，没配就只在窗口里
   真能看见那条边界时才认（第一个有 codex 记账的周前面还存在一个没有的周），否则为空——
   **宁可不标，也不能凭空造一个新边界**。四周过渡期按真实切换日期算周数差，不按窗口下标。
