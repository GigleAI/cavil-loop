...
| 夹紧 | **每一个**数值字段都验 —— 字符类**和位数**一起验，因为 bash 算术会把位数超标的全数字值静默回绕成一个看着正常的时间戳 —— 而且必须在任何「等一等」的决定**之前**验完。缺失 / 读不懂 / 超范围一律当作「现在就该跑」，文件在下一轮真跑时重写成干净值。只验其中几个字段是同一个 bug 的缩小版：一个验得晚的坏字段照样能让本轮等下去 |
