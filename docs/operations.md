...
├── .deploy.lock                        # shared by all project instances
...
The poll entry, token-usage drivers, and weekly-report entry pin the physical release before reading sibling files and hold its shared `.inuse` lease until exit. Cleanup removes an old release only after taking that lease exclusively; age and “keep N versions” are deliberately not used. Optional shared overrides live in `~/.config/coding-agent-work-loop/deploy.conf`; see `coding-agent-deploy.conf.example`.
...
Managed **Linux** polls fetch the configured base branch, unpack a complete immutable tree, then atomically replace the stable link. Offline, lock, and fetch failures retain the current release and do not block polling. Migrate a checkout-style installation only through an explicit action:
...
After the remote is observed ahead for the configured threshold, deployment opens at most one issue in the configured alert repository and closes it after recovery.
