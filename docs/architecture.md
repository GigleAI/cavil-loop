...
| Clamping | **Every** numeric field is validated — character class *and* digit width, since bash arithmetic silently wraps an oversized all-digit value into a plausible-looking timestamp — before any decision to wait. Missing, unparseable or out-of-range means "due now"; the file is rewritten clean on the next poll. Validating only some of the fields is the same bug in a smaller size: a corrupt one that is checked late still lets the tick wait. |
