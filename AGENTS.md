...
- **Don't round at the serialization boundary.** Two decimals are for humans; the numbers in the machine record are for the collector. Formatting a trust bucket with the human formatter turned \$0.0025 into `unstable:0.00` — the state was destroyed at the source, so the fallback path (log deleted, footer reused) lost it entirely and no downstream fix could bring it back. Keep sub-cent values at full precision in machine fields, and keep a true zero written as zero — never inflate it into a small positive amount.
