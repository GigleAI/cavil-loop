...
description: 用 GitHub label 驱动本机 agent 处理 issue/PR，daemon 自动监听并派工。设计方案 / 讨论 / 代码 / Claude 对话 / tmux 历史全程持久化，事后可追溯、可断点续写。包含 setup（一次部署到某个项目）、status（查 daemon 状态）、disable（关闭某项目的 daemon）等命令
---
